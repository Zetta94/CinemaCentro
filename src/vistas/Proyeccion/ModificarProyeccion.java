...
public class ModificarProyeccion extends javax.swing.JInternalFrame {
    ...
    public ModificarProyeccion(Proyeccion proyeccion) {
    ...
    private void llenarCampos() {
        System.out.println(proyeccion.getSala().getIdSala());
        for (int i = 0; i < cbxPelicula.getItemCount(); i++) {
            Pelicula item = cbxPelicula.getItemAt(i);
            if (item.getIdPelicula() == proyeccion.getPelicula().getIdPelicula()) {
                cbxPelicula.setSelectedIndex(i);
                break;
            }
        }
        for (int i = 0; i < cbxSala.getItemCount(); i++) {
            Sala item = cbxSala.getItemAt(i);
            if (item.getIdSala() == proyeccion.getSala().getIdSala()) {
                cbxSala.setSelectedIndex(i);
                break;
            }
        }
        cbxIdioma.setSelectedItem(proyeccion.getIdioma());
        dateFecha.setDate(java.sql.Date.valueOf(proyeccion.getFecha()));
        spinInicio.setValue(java.sql.Time.valueOf(proyeccion.getHoraInicio()));
        spinFin.setValue(java.sql.Time.valueOf(proyeccion.getHoraFin()));
        if (proyeccion.isEs3D()) {
            bSi.setSelected(true);
        } else {
            bNo.setSelected(true);
        }
        if (proyeccion.isSubtitulada()) {
            bSi1.setSelected(true);
        } else {
            bNo1.setSelected(true);
        }
        txtPrecio.setText(String.valueOf(proyeccion.getPrecio()));
    }
    ...
    private void btnGurardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnGurardarActionPerformed
        ...
        proy.setIdProyeccion(proyeccion.getIdProyeccion());
        proy.setSala(sala);
        ...
        boolean creada = Context.getProyeccionData().modificarProyeccion(proy);
